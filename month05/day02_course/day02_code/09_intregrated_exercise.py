...
class GuaZiSpider:
    def __init__(self):
        ...
        # mysql 连接
        self.mysql_db = pymysql.connect('localhost', 'root', '417355570', 'guazidb', charset='utf8')
    ...
    def parse_html(self, first_url):
        ...
        for r in rlist:
            item = {}
            item['href'] = 'https://www.guazi.com' + r.strip()  # 注意这里的细节，斜杠要不要写
            # print(item['href'])
            finger = self.hash_gen(item)
            res = self.r.sadd('guazi', finger)
            if res == 1:
    ...
    def get_second_data(self, second_url, item):
        ...
        for second in second_rlist:
            ...
            print(second_item)
            self.save_mongo(second_item)
            self.save_mysql(second_item)
            self.save_csv(second_item)
    ...
    def save_mysql(self, item):
        ins = 'insert into guazi values(%s,%s)'
        self.cur.execute(ins, [item['href'], item['title']])

    def save_csv(self, item):
        self.writer.writerow([item['href'], item['title']])

    def hash_gen(self, item):
        md5 = hashlib.md5()
        md5.update(item['href'].encode())  # encode 不要忘！！
        finger = md5.hexdigest()
        return finger
    ...
    def close(self):
        ...
        self.mysql_db.close()
        # mongodb
        # mongodb不需要关闭，会自动维护连接池；不需要提交，没有事务机制
        # csv
        self.f.close()
        # redis
