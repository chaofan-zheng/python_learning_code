...
class ProxyPool:
    ...
    def test_proxy(self, proxy):
        ...
        try:
            resp = requests.get(url=self.test_url, proxies=proxies, headers={'User-Agent': UserAgent().random},
                                timeout=3)
            # print(f'{proxy}可用！！！！')
    ...
    def get_ip(self, start, end, res_ip_list):
        ...
        for i in range(start, end):
            ...
            try:
                response = requests.get(url=url, headers={'User-Agent': UserAgent().random}, timeout=3).text
            except Exception as e:
                # print(e)
                continue
            # print(response)
...
if __name__ == '__main__':
    ...
    # response = requests.get(url='https://www.kuaidaili.com/free/inha/1/', headers={'User-Agent':UserAgent().random}, timeout=5).text
    # print(response)
    proxies = {
        'http': 'http://{}'.format('223.247.169.206:9999'),
        'https': 'https://{}'.format('223.247.169.206:9999'),
    }
    try:
        resp = requests.get(url='http://httpbin.org/get', proxies=proxies, headers={'User-Agent': UserAgent().random},
                            timeout=3)
        print(f'{"223.247.169.206:9999"}可用！！！！')
        # self.useful_ip.append(proxy)
    except Exception as e:
        print(f'{"223.247.169.206:9999"}不可用')
        pass